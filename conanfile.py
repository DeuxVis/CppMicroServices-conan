...
class CppMicroServicesConan(ConanFile):
    ...
    def build(self):
        ...
        if self.settings.arch == 'x86_64':
            option_defines += ' -DCMAKE_GENERATOR_PLATFORM=x64'

        option_defines += " -DCMAKE_BUILD_TYPE={type}".format(type=self.settings.build_type)

        self.run("cmake {src_dir} -B{build_dir} {defines}".format(src_dir=self.name,
                                                                  build_dir=self.build_dir,
                                                                  defines=option_defines))
        self.run("cmake --build {build_dir} --config {type}".format( build_dir=self.build_dir, type=self.settings.build_type))
    ...
    def package(self):
        ...
        #print ("### Copying {type} binaries from build_bin_dir = '{var}' :".format(  type=self.settings.build_type, var=build_bin_dir) )
        self.copy('*.so*', dst='lib', src=build_bin_dir)
        self.copy('*.dll', dst='bin', src=build_bin_dir)
        
        # Executables
        self.copy('usResourceCompiler3*', dst='bin', src=build_bin_dir)
        self.copy('usShell3*', dst='bin', src=build_bin_dir)

        self.copy('GettingStarted*', dst='bin', src=build_bin_dir)
        self.copy('usTutorialDriver*', dst='bin', src=build_bin_dir)
        self.copy('usWebConsoleDriver*', dst='bin', src=build_bin_dir)
